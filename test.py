...
fashiongen = FashionGenDataset(validation_dataset_path)
...
class FashionGenTorchDataset(torch.utils.data.Dataset):
    ...
    def get_product_same_category(self, index: int):
        product = fashiongen.get_product(index)
        if self.subcategory:
            product.subcategory = product.subcategory.encode("ISO-8859-9")
            similiar = fashiongen.get_same_subcategory_of(product)[0].image
        else:
            product.category = product.category.encode("ISO-8859-9")
            similiar = fashiongen.get_same_category_of(product)[0].image
        return self.preprocess_image(similiar).to(self.device)
...
def get_metrics(model_label: str, load_metrics: bool = False):
    ...
    # METRICS
    if load_metrics:
        # with open(os.path.join(drive_path,, "rb") as file:
        scores = np.load(f"predictions/metrics-{model_label}.npy", allow_pickle=True)
    else:
        scores = {}
        scores["bleu"] = []
        scores["rougeL"] = []
        ...
        num_captions = len(data.caption.values)
        for i in tqdm(range(0, num_captions)):
        ...
        np.save(f"predictions/metrics-{model_label}.npy", scores)
...
def print_metrics(model_label: str):
    # TRIPLET
    # with open(os.path.join(drive_path, f"predictions/metrics-{model_label}.npy"), "rb") as file:
    metrics_triplet = np.load(f"predictions/metrics-{model_label}.npy", allow_pickle=True).item()
    ...
    #with open(os.path.join(drive_path, f"predictions/metrics-{model_label}.npy"), "rb") as file:
    metrics_entropy = np.load(f"predictions/metrics-{model_label}.npy", allow_pickle=True).item()
...
if print_metrics:
    print_metrics(model_label=checkpoint_name)
